...
export class AnalyticsService {
   ...
   */
  public calculateSummary(reviews: AIReview[]): Summary {
   ...
   */
  public calculateSentimentScore(reviews: AIReview[]): SentimentScore {
    const totalReviews = reviews.length;
    if (totalReviews === 0) {
      return {
        positive: 0,
   ...
   */
  public calculateComplaintCategories(reviews: AIReview[]): ComplaintCategory[] {
    const totalReviews = reviews.length;
    if (totalReviews === 0) {
      return [];
    }
   ...
   */
  private getSentimentCounts(reviews: AIReview[]): {
    positive: number;
    neutral: number;
    negative: number;
  } {
