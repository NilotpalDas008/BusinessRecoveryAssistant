...
describe("GeminiService REST API Transport", () => {
  ...
  it("should throw GeminiAuthError if GEMINI_API_KEY is missing", async () => {
    ...
    await expect(service.generateJSON("test")).rejects.toThrow(GeminiAuthError);
  });
  ...
  it("should send correct POST request format and parse JSON response", async () => {
    ...
    const service = new GeminiService();
    const result = await service.generateJSON<{ success: boolean }>(
      "Hello prompt",
    ...
    expect(parsedBody.contents[0].parts[0].text).toBe(
      "System:\nAct as helpful assistant\n\nUser:\nHello prompt"
    );
  });
  ...
  it("should format body without system prefix if systemInstruction is omitted", async () => {
    ...
    const service = new GeminiService();
    const result = await service.generateJSON<{ result: number }>("Simple prompt");
  ...
  it("should map HTTP 429 response to GeminiRateLimitError", async () => {
    ...
    const service = new GeminiService();
    await expect(service.generateJSON("test")).rejects.toThrow(GeminiRateLimitError);
  });
  ...
  it("should map invalid JSON output from model to GeminiInvalidJsonError", async () => {
    ...
    const service = new GeminiService();
    await expect(service.generateJSON("test")).rejects.toThrow(GeminiInvalidJsonError);
  });
});
